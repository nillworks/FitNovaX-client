    ...
    <div className="flex flex-col gap-6 h-full">
      <div className="flex flex-col gap-2">
        <div className="flex justify-between items-center">
          <span className="text-[#1E293B] font-semibold">Application Status</span>
        ...
        <div className="w-full bg-[#E2E8F0] rounded-full h-2.5 mt-2">
          <div className="bg-[#22C55E] h-2.5 rounded-full" style={{ width: `${application.progress}%` }}></div>
        </div>
        <p className="text-xs text-[#64748B] mt-1 text-right">{application.step}</p>
      </div>
      ...
      <div className="bg-[#F8FAFC] p-4 rounded-2xl border border-[#E2E8F0] flex flex-col gap-4">
        ...
        {application.documents.map((doc, idx) => (
          <div key={idx} className="flex items-center justify-between text-sm bg-white p-3 rounded-xl border border-[#E2E8F0]">
            ...
            <span className={`${doc.statusColor} font-semibold text-xs ${doc.statusBg} px-2 py-1 rounded`}>
              {doc.status}
            </span>
          </div>
        ))}
