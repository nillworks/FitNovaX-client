...
const MyClassesRow = ({ data }) => {
  ...
  const date =
    data.startDate && data.endDate
      ? `${formatDate(data.startDate)} To ${formatDate(data.endDate)} end`
      : data.date || 'No Date';
  const time =
  ...
  const maxStudents = data.maxBookings
    ? parseInt(data.maxBookings)
    : (data.maxStudents ?? 1);
  ...
  const isStatusActive =
    classStatus?.toLowerCase() === 'active' ||
    classStatus?.toLowerCase() === 'approved';
