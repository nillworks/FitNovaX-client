...
const formatDate = (dateString) => {
  ...
  return `${actualMonths[d.getMonth()]} ${d.getDate()} ${d.getFullYear()}`;
};

const formatDuration = (mins) => {
  if (!mins) return '';
  const totalMins = parseInt(mins, 10);
  if (isNaN(totalMins)) return mins;
...
const MyClassesRow = ({ data }) => {
  const name = data.className || data.name || 'Untitled Class';
  const date = (data.startDate && data.endDate) ? `${formatDate(data.startDate)} To ${formatDate(data.endDate)} end` : data.date || 'No Date';
  const time = (data.startTime && data.endTime) ? `${data.startTime} - ${data.endTime}` : data.time || 'No Time';
  const students = data.bookedCount ?? data.students ?? 0;
  const maxStudents = data.maxBookings ? parseInt(data.maxBookings) : (data.maxStudents ?? 1);
  const classStatus = data.status || 'Active';
  ...
  const isStatusActive = classStatus?.toLowerCase() === 'active' || classStatus?.toLowerCase() === 'approved';
  const isStatusPending = classStatus?.toLowerCase() === 'pending';
  ...
  return (
    <div className="group bg-[#FFFFFF] border border-[#E2E8F0] rounded-3xl p-6 transition-all duration-300 hover:shadow-xl hover:-translate-y-1 relative flex flex-col h-full overflow-hidden">
      ...
      {classImage ? (
        <>
          <div 
            className="absolute top-0 left-0 w-full h-32 bg-cover bg-center pointer-events-none opacity-20" 
            style={{ backgroundImage: `url(${classImage})` }}
          ></div>
          <div className="absolute top-0 left-0 w-full h-32 bg-gradient-to-b from-transparent to-white pointer-events-none"></div>
        </>
      ) : (
      ...
      <div className="flex justify-between items-start mb-4 relative z-10">
        <span
          className={`inline-flex items-center px-3.5 py-1.5 rounded-full text-xs font-bold tracking-tight uppercase shadow-sm border 
            ${isStatusActive ? 'bg-[#22C55E] text-white border-[#16A34A]' : 
              isStatusPending ? 'bg-[#EAB308] text-white border-[#CA8A04]' : 
      ...
      <div className="relative z-10 flex-grow flex flex-col">
        <h3 className="text-xl font-bold text-[#1E293B] tracking-tight mb-5 line-clamp-2 leading-tight group-hover:text-[#16A34A] transition-colors">
          {name}
        </h3>
        ...
        <div className="space-y-3.5 mb-6">
          ...
          {price && (
            <div className="flex items-center text-[#64748B] text-sm font-medium">
              <DollarSign className="w-4 h-4 text-[#4AD27A] mr-3" />
              Price: ${price}
            </div>
          )}
          {category && (
            <div className="flex items-center text-[#64748B] text-sm font-medium capitalize">
              <Activity className="w-4 h-4 text-[#4AD27A] mr-3" />
              Category: {category}
            </div>
          )}
        </div>
