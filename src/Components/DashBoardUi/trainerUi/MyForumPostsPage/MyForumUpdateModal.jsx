...
import { UploadCloud, FileText, CheckCircle2, X, Edit2, Image as ImageIcon } from 'lucide-react';
...
const MyForumUpdateModal = ({ isOpen, onClose, initialData, onSubmit }) => {
  ...
  useEffect(() => {
    if (initialData && isOpen) {
      ...
      setSummaryLength((initialData.summary || initialData.shortDescription || '').length);
  ...
  const handleFormSubmit = (e) => {
    ...
    const updateForum = {
      ...
      category,
      tags: selectedTags,
      coverImage: imagePreview, // Use preview or handle actual file upload in parent
      imageFile, // Pass the file in case the parent wants to upload it
      summary,
      description,
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[75vh] overflow-hidden flex flex-col z-10 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="overflow-y-auto custom-scrollbar p-5 md:p-6 flex-1">
          <form id="updateForumForm" onSubmit={handleFormSubmit} className="space-y-8">
            
            {/* Section 1: Post Basics */}
            <section>
              <div className="mb-4 border-b border-[#E2E8F0] pb-2">
                <h3 className="text-base font-bold text-[#1E293B]">Post Basics</h3>
              </div>
            ...
            <section>
              <div className="mb-4 border-b border-[#E2E8F0] pb-2 mt-2">
                <h3 className="text-base font-bold text-[#1E293B]">Cover Image</h3>
              </div>
              <div
              ...
              >
                ...
                {imagePreview ? (
                  ...
                  <div className="flex flex-col items-center justify-center">
                    ...
                    <h3 className="text-sm font-bold text-[#1E293B] mb-1">Click to change cover</h3>
                    <p className="text-[#64748B] text-xs font-medium">PNG, JPG, WebP up to 5MB</p>
                  </div>
                )}
              </div>
            </section>
            ...
            <section>
              <div className="mb-4 border-b border-[#E2E8F0] pb-2 mt-2">
                <h3 className="text-base font-bold text-[#1E293B]">Content Details</h3>
              </div>
              <div className="space-y-5">
                <InputWrapper 
                  label="Short Summary" 
                  required
                  extra={<span className={`text-xs font-bold px-2 py-1 rounded-md ${summaryLength > 150 ? 'bg-red-100 text-red-600' : 'bg-[#F8FAFC] text-[#64748B]'}`}>{summaryLength} / 150</span>}
                >
                  <textarea
                    rows="2"
                    required
            ...
            <section>
              <div className="mb-4 border-b border-[#E2E8F0] pb-2 mt-2">
                <h3 className="text-base font-bold text-[#1E293B]">Publishing Settings</h3>
              </div>
                ...
                <div>
                  ...
                  >
                    <div className="flex flex-col">
                      <span className={`font-bold text-sm ${isFeatured ? 'text-[#15803D]' : 'text-[#1E293B]'}`}>Pin to Top</span>
                    </div>
                    <div className={`relative w-12 h-6 rounded-full transition-colors duration-300 shadow-inner ${isFeatured ? 'bg-[#22C55E]' : 'bg-[#E2E8F0]'}`}>
                      <div className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow-sm transition-transform duration-300 ${isFeatured ? 'left-7' : 'left-1'}`}></div>
                    </div>
                  </div>
                </div>
              </div>
